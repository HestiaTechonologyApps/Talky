...
class InvoiceMasterService {
  static async getAll(): Promise<CustomResponse<Invoice[]>> {
    return HttpService.callApi(API_ENDPOINTS.INVOICE_MASTER.GET_ALL, "GET");
  }

  static async getById(id: number): Promise<CustomResponse<Invoice>> {
    return HttpService.callApi(API_ENDPOINTS.INVOICE_MASTER.GET_BY_ID(id), "GET");
  }

  static async create(data: Invoice): Promise<CustomResponse<Invoice>> {
    return HttpService.callApi(API_ENDPOINTS.INVOICE_MASTER.CREATE, "POST", data);
  }

  static async update(id: number, data: Invoice): Promise<CustomResponse<Invoice>> {
